...
public class m_20DAO {
	/**
	 * 获取题目
	 * @param id
	 * @return
	 */
	public static m_20 getQuestion(int id){
		m_20 sel=null;
		...
		try {
			...
			LogUtil.Error("m_20DAO中获取题目失败 "+e.getMessage());
			JOptionPane.showMessageDialog(null,"检查数据库连接","提示",JOptionPane.PLAIN_MESSAGE);
//			e.printStackTrace();
			return null;
		} catch (Exception e) {
			LogUtil.Error("m_20DAO中修改Flag不成功 "+e.getMessage());
//			e.printStackTrace();
		}finally{
	...
	public static boolean setFlag(int id){
		...
		Connection conn=null;
		PreparedStatement ps=null;
		String sql="UPDATE m_20 SET flag=1 WHERE id=?";
		try {
			conn=DBUtil.getConnection();
			ps=conn.prepareStatement(sql);
			ps.setInt(1, id);
			...
			LogUtil.Error("m_20DAO中修改Flag失败 "+e.getMessage());
			JOptionPane.showMessageDialog(null,"检查数据库连接","提示",JOptionPane.PLAIN_MESSAGE);
//			e.printStackTrace();
		}finally{
			DBUtil.closeConnection(conn);
	...
	public static boolean resetFlag(){
		...
		Connection conn=null;
		PreparedStatement ps=null;
		String sql="UPDATE m_20 SET flag=0";
		try {
			conn=DBUtil.getConnection();
			ps=conn.prepareStatement(sql);
			int t=ps.executeUpdate();
			...
			LogUtil.Error("m_20DAO中重置Flag失败 "+e.getMessage());
			JOptionPane.showMessageDialog(null,"检查数据库连接","提示",JOptionPane.PLAIN_MESSAGE);
//			e.printStackTrace();
		}finally{
			DBUtil.closeConnection(conn);
	...
	public static boolean addQuestion(String question, String answerA, String answerB,
		...
		Connection conn=null;
		PreparedStatement ps=null;
		String sql="INSERT INTO m_20 (question,answerA,answerB,answerC,answerD,answer,flag) VALUES (?,?,?,?,?,?,0);";
		try {
			conn=DBUtil.getConnection();
			ps=conn.prepareStatement(sql);
			...
			LogUtil.Error("m_20DAO中添加题目失败 "+e.getMessage());
			JOptionPane.showMessageDialog(null,"检查数据库连接","提示",JOptionPane.PLAIN_MESSAGE);
//			e.printStackTrace();
		}finally{
			DBUtil.closeConnection(conn);
	...
	public static int getCount(){
		...
		Connection conn=null;
		PreparedStatement ps=null;
		String sql="SELECT COUNT(*) count FROM m_20;";
		try {
			conn=DBUtil.getConnection();
			ps=conn.prepareStatement(sql);
			ResultSet rs=ps.executeQuery();
			...
			LogUtil.Error("m_20DAO中查询count失败 "+e.getMessage());
			JOptionPane.showMessageDialog(null,"检查数据库连接","提示",JOptionPane.PLAIN_MESSAGE);
//			e.printStackTrace();
		}finally{
			DBUtil.closeConnection(conn);
