...
public class GetFilling extends JPanel {
	public static int[]fillingSerial=null;
...
//					do{
						--index;
						int id=fillingSerial[index];
						fill=FillingDAO.getQuestion(id);
//					}while(fill.getFlag()==1);
		...
		getNext.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				if(index<fillingSerial.length-1){
//					do{
						++index;
						int id=fillingSerial[index];
						fill=FillingDAO.getQuestion(id);
